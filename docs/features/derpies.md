...
- **Repeat-image fast path**: an image whose CONTENT (sha256 of the downloaded bytes) was already LLM-judged is a repeat, not new content — a message whose content is image-only and whose images are all seen is deleted with ZERO asks (log `derpies delete (repeat image)`); seen images drop out of mixed asks (fresh images + text are still judged, seen ones leave the payload). An ask failure marks nothing, so an un-judged image is always judged and never fast-deleted blind. The seen-content cache is in-process only (24h TTL, bounded, oldest-evicted): a bot restart re-judges a first sighting once before it fast-deletes again.
